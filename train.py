...
parser.add_argument('--method', type=str, default='CASF_SST')
...
parser.add_argument("--outf", type=str, default='./exp/CASF_SST/loss', help='path log files')
...
criterion_EdgeLoss = losses.EdgeLoss()
criterion_Charbonnier = losses.CharbonnierLoss()
criterion_ColourDistance = losses.ColourDistance()
...
if torch.cuda.is_available():
    ...
    criterion_rmse.cuda()
    criterion_psnr.cuda()
    criterion_EdgeLoss.cuda()
    criterion_Charbonnier.cuda()
    criterion_ColourDistance.cuda()
    criterion_l1.cuda()
    criterion_mse.cuda()
...
def main():
    ...
    while iteration<total_iteration:
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            output = model(images)
            # loss_char = np.sum([criterion_Charbonnier(output[j], labels) for j in range(len(output))])
            # loss_edge = np.sum([criterion_EdgeLoss(output[j], labels) for j in range(len(output))])
            # loss_edge = criterion_EdgeLoss(output, labels)
            # loss_char = criterion_Charbonnier(output, labels)
            # loss_color = criterion_ColourDistance(output, labels)
            # loss = (loss_char*0.5)+(0.05*loss_edge)+(loss_color)
